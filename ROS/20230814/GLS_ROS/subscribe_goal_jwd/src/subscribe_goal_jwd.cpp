...
#include "subscribe_goal_jwd/Subscribe_goal_jwd.h"
#include <thread>
...
void GoalCallback(const subscribe_goal_jwd::Subscribe_goal_jwd& msg) {
...
void publishThread() {
    ros::NodeHandle nh;
    action_state_number_pub = nh.advertise<std_msgs::Int32>("/action_state_number_jwd", 1000);
...
void subscribeThread() {
    ros::NodeHandle nh;
    // 创建一个Subscriber，订阅名为/subscribe_goal_jwd，注册回调函数GoalCallback
    ros::Subscriber goal_sub = nh.subscribe("/subscribe_goal_jwd", 10, GoalCallback);
    ROS_INFO("Subscribe /subscribe_goal_jwd successfully!");
    ros::spin();
}

int main(int argc, char** argv){
    ros::init(argc, argv, "subscribe_goal_jwd");
    ROS_INFO("node subscribe_goal_jwd launch successfully!!!");
